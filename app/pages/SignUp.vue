      ...
      <div class="relative max-w-6xl mx-auto px-4 sm:px-6">
        <div class="pt-32 pb-12 md:pt-40 md:pb-20">
          ...
          <div class="max-w-3xl mx-auto text-center pb-12">
            ...
            <!-- Page title -->
            <h1 class="h2 bg-clip-text text-transparent bg-linear-to-r from-slate-200/60 via-slate-200 to-slate-200/60">{{$t('app/pages/SignUp.vue:Create your free account')}}</h1>
          </div>
          ...
          <div class="max-w-sm mx-auto">
            ...
            <form>
              <div class="space-y-4">
                <div>
                  <label class="block text-sm text-slate-300 font-medium mb-1" for="company">Company <span class="text-rose-500">*</span>{{$t('app/pages/SignUp.vue:*')}}</label>
                  <input id="company" class="form-input w-full" type="text" placeholder="mE.g., Acme Inc." required />
                </div>
                <div>
                  <label class="block text-sm text-slate-300 font-medium mb-1" for="full-name">Full Name <span class="text-rose-500">*</span>{{$t('app/pages/SignUp.vue:*')}}</label>
                  <input id="full-name" class="form-input w-full" type="text" placeholder="E.g., Mark Rossi" required />
                </div>
                <div>
                  <label class="block text-sm text-slate-300 font-medium mb-1" for="email">Email <span class="text-rose-500">*</span>{{$t('app/pages/SignUp.vue:*')}}</label>
                  <input id="email" class="form-input w-full" type="email" placeholder="markrossi@company.com" required />
                </div>
                <div>
                  <label class="block text-sm text-slate-300 font-medium mb-1" for="password">Password <span class="text-rose-500">*</span>{{$t('app/pages/SignUp.vue:*')}}</label>
                  <input id="password" class="form-input w-full" type="password" autocomplete="on" required />
                </div>
                <div>
                  <label class="block text-sm text-slate-300 font-medium mb-1" for="referrer">Where did you hear about us? <span class="text-rose-500">*</span>{{$t('app/pages/SignUp.vue:*')}}</label>
                  <select id="referrer" class="form-select text-sm py-2 w-full" required>
                    <option>{{$t('app/pages/SignUp.vue:Google')}}</option>
                    <option>{{$t('app/pages/SignUp.vue:Medium')}}</option>
                    <option>{{$t('app/pages/SignUp.vue:GitHub')}}</option>
                  </select>
                </div>
              </div>
              <div class="mt-6">
                <button class="btn text-sm text-white bg-purple-500 hover:bg-purple-600 w-full shadow-xs group">
                  Sign Up <span class="tracking-normal text-purple-300 group-hover:translate-x-0.5 transition-transform duration-150 ease-in-out ml-1">{{$t('app/pages/SignUp.vue:-&gt;')}}</span>
                </button>
              </div>
            </form>
            ...
            <div class="text-center mt-4">
              <div class="text-sm text-slate-400">
                Already have an account? <router-link class="font-medium text-purple-500 hover:text-purple-400 transition duration-150 ease-in-out" to="/signin">{{$t('app/pages/SignUp.vue:Sign in')}}</router-link>
              </div>
            </div>

            <!-- Divider -->
            <div class="flex items-center my-6">
              <div class="border-t border-slate-800 grow mr-3" aria-hidden="true">{{$t('app/pages/SignUp.vue:*')}}</div>
              <div class="text-sm text-slate-500 italic">{{$t('app/pages/SignUp.vue:or')}}</div>
              <div class="border-t border-slate-800 grow ml-3" aria-hidden="true">{{$t('app/pages/SignUp.vue:*')}}</div>
            </div>
            ...
            <div class="flex space-x-3">
              <button class="btn text-slate-300 hover:text-white transition duration-150 ease-in-out w-full group [background:linear-gradient(var(--color-slate-900),var(--color-slate-900))_padding-box,conic-gradient(var(--color-slate-400),var(--color-slate-700)_25%,var(--color-slate-700)_75%,var(--color-slate-400)_100%)_border-box] relative before:absolute before:inset-0 before:bg-slate-800/30 before:rounded-full before:pointer-events-none h-9">
                <span class="relative">
                  <span class="sr-only">{{$t('app/pages/SignUp.vue:Continue with Twitter')}}</span>
              ...
              <button class="btn text-slate-300 hover:text-white transition duration-150 ease-in-out w-full group [background:linear-gradient(var(--color-slate-900),var(--color-slate-900))_padding-box,conic-gradient(var(--color-slate-400),var(--color-slate-700)_25%,var(--color-slate-700)_75%,var(--color-slate-400)_100%)_border-box] relative before:absolute before:inset-0 before:bg-slate-800/30 before:rounded-full before:pointer-events-none h-9">
                <span class="relative">
                  <span class="sr-only">{{$t('app/pages/SignUp.vue:Continue with GitHub')}}</span>
